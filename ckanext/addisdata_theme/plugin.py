...
class addisdataThemePlugin(plugins.SingletonPlugin, DefaultTranslation):
    ...
    def update_config(self, config_):
        ...
        toolkit.add_resource('fanstatic', 'addisdata_theme')
    ...
    #     GA_CONTROLLER = """
    #         ckanext.addisdata_theme.controllers.gacontroller:GAController"""
    ...
    def update_config_schema(self, schema):
        ...
        schema.update({
            'ckanext.addisdata_theme.disallowed_extensions': [ignore_missing, str],
            'ckanext.addisdata_theme.allowed_extensions': [ignore_missing, str],
            # 'groups': [not_empty],
        })
    ...
    def before_create(self, context, resource):
        disallowed_extensions = toolkit.aslist(config.get('ckanext.addisdata_theme.disallowed_extensions',[]))
        disallowed_mimetypes = [mimetypes.types_map["." + x] for x in disallowed_extensions]

        allowed_extensions = toolkit.aslist(config.get('ckanext.addisdata_theme.allowed_extensions',[]))
    ...
    def before_show(context, resource_dict):
        custom_url = config.get('addisdata_theme.custom_resource_download_url')
        ...
        if custom_url:
            ...
            resource_dict['addisdata_download_url'] = resource_dict['url'].replace(old_url, custom_url)
        else:
            resource_dict['addisdata_download_url'] = resource_dict['url']
